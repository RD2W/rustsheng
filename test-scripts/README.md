...
# Для V3/K1 — можно явно указать CPU (если автоопределение не справляется):
...
- Прошивка отклоняется при несовпадении CPU образа и загрузчика радио. Для нестандартных сборок используйте `--force-cpu`.
