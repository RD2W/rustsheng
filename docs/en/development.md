...
formats, AES key/IV table and `pack` logic come from `K5TOOL`. The per-CPU
flash-size limits protect the bootloader from being overwritten; the bootloader-version to CPU mapping (`Cpu::from_boot_version`) is derived from forum reports of the TWHH (OUROBOROS) flasher suite; per-CPU EEPROM range limits mirror K5TOOL's `MinEepromAddr`/`MaxEepromAddr`. The PY32 PID mapping
